...
async def create_prediction(
    ...
    result = await db.execute(
        select(Dataset).where(Dataset.id == pred_in.dataset_id, Dataset.owner_id == current_user.id)
    )
...
async def run_prediction(pred_id, file_path, file_type, model_type, target_col, feature_cols, config):
    ...
    async with AsyncSessionLocal() as db:
        ...
        if not pred:
            return
        try:
            output = await ml_engine.run_pipeline(file_path, file_type, model_type, target_col, feature_cols, config)
...
async def list_predictions(
    ...
    q = select(Prediction).where(Prediction.owner_id == current_user.id).offset(skip).limit(limit)
...
async def get_prediction(
    ...
    result = await db.execute(
        select(Prediction).where(Prediction.id == pred_id, Prediction.owner_id == current_user.id)
    )
    pred = result.scalar_one_or_none()
    if not pred:
